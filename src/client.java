...
public class client extends RSApplet {
	...
	private void connectServer() {
		/*
		 * int j = 5; expectedCRCs[8] = 0; int k = 0; while(expectedCRCs[8] == 0) { String s = "Unknown problem"; drawLoadingText(20, (byte)4, "Connecting to web server"); try { DataInputStream datainputstream = openJagGrabInputStream("crc" + (int)(Math.random() * 99999999D) + "-" + 317); Stream class30_sub2_sub2 = new Stream(new byte[40], 891); datainputstream.readFully(class30_sub2_sub2.buffer, 0, 40); datainputstream.close(); for(int i1 = 0; i1 < 9; i1++) expectedCRCs[i1] = class30_sub2_sub2.readDWord();
		 * 
		 * int j1 = class30_sub2_sub2.readDWord(); int k1 = 1234; for(int l1 = 0; l1 < 9; l1++) k1 = (k1 << 1) + expectedCRCs[l1];
		 * 
		 * if(j1 != k1) { s = "checksum problem"; expectedCRCs[8] = 0; } } catch(EOFException _ex) { s = "EOF problem"; expectedCRCs[8] = 0; } catch(IOException _ex) { s = "connection problem"; expectedCRCs[8] = 0; } catch(Exception _ex) { s = "logic problem"; expectedCRCs[8] = 0; if(!signlink.reporterror) return; } if(expectedCRCs[8] == 0) { k++; for(int l = j; l > 0; l--) { if(k >= 10) { drawLoadingText(10, (byte)4, "Game updated - please reload page"); l = 10; } else { drawLoadingText(10, (byte)4, s + " - Will retry in " + l + " secs."); } try { Thread.sleep(1000L); } catch(Exception _ex) { } }
		 * 
	...
	void startUp() {
		...
		try {
			...
			for (int i8 = 0; i8 < 9; i8++) {
				int k8 = 128 + i8 * 32 + 15;
				int l8 = 600 + k8 * 3;
				float i9 = Texture.anIntArray1470[k8];
				ai[i8] = (int) (l8 * i9);
			}
		...
		for (int j2 = 0; j2 < i2; j2++) {
			...
			if (class9_1.type == 0) {
				...
				if (class9_1.type == 2) {
					...
					float i5 = Texture.anIntArray1470[class9_1.anInt270] * class9_1.anInt269;
					float l5 = Texture.anIntArray1471[class9_1.anInt270] * class9_1.anInt269;
	...
	private void calcEntityScreenPos(float i, int j, float l) {
		if (i < 128 || l < 128 || i > 13056 || l > 13056) {
			spriteDrawX = -1;
			spriteDrawY = -1;
			return;
		}
		float i1 = method42(plane, (int) l, (int) i) - j;
		i -= xCameraPos;
		i1 -= zCameraPos;
		l -= yCameraPos;
		float j1 = Model.modelIntArray1[yCameraCurve];
		float k1 = Model.modelIntArray2[yCameraCurve];
		float l1 = Model.modelIntArray1[xCameraCurve];
		float i2 = Model.modelIntArray2[xCameraCurve];
		float j2 = l * l1 + i * i2;
		l = l * i2 - i * l1;
		i = j2;
		j2 = i1 * k1 - l * j1;
		...
		i1 = j2;
		if (l >= 50) {
			spriteDrawX = (int) (Texture.textureInt1 + (i * 512.0F) / l);
			spriteDrawY = (int) (Texture.textureInt2 + (i1 * 512.0F) / l);
		} else {
			spriteDrawX = -1;
			spriteDrawY = -1;
		}
	...
	private void markMinimap(Sprite sprite, int i, int j) {
		...
		if (l > 6400)
			return;
		float i1 = Model.modelIntArray1[k];
		float j1 = Model.modelIntArray2[k];
		int k1 = (int) (j * i1 + i * j1);
		int l1 = (int) (j * j1 - i * i1);
		if (l > 2500) {
	...
	private void setCameraPos(int j, int k, int l, int i1, int j1, int k1) {
		...
		float l2 = j;
		if (l1 != 0) {
			float i3 = Model.modelIntArray1[l1];
			float k3 = Model.modelIntArray2[l1];
			float i4 = k2 * k3 - l2 * i3;
			l2 = k2 * i3 + l2 * k3;
			k2 = i4;
		}
		if (i2 != 0) {
			float j3 = Model.modelIntArray1[i2];
			float l3 = Model.modelIntArray2[i2];
			float j4 = l2 * j3 + j2 * l3;
			l2 = l2 * l3 - j2 * j3;
			j2 = j4;
		}
		xCameraPos = (int) (l - j2);
		zCameraPos = (int) (i1 - k2);
