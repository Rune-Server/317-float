...
public final class ObjectDef {
	...
	public Model method578(int i, int j, int k, int l, int i1, int j1, int k1) {
		...
		if (aBoolean762) {
			...
			for (int i2 = 0; i2 < model.anInt1626; i2++) {
				float j2 = model.anIntArray1627[i2];
				float k2 = model.anIntArray1629[i2];
				float l2 = k + ((l - k) * (j2 + 64)) / 128;
				float i3 = j1 + ((i1 - j1) * (j2 + 64)) / 128;
				float j3 = l2 + ((i3 - l2) * (k2 + 64)) / 128;
	...
	private Model method581(int j, int k, int l) {
		...
		if (anInt760 == 1)
			model_3.anInt1654 = (int) model_3.modelHeight;
